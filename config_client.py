    ...
    shortcuts = [
        {
            'key': 'f7',            # F7 键
            'type': 'keyboard',     # 键盘快捷键
            'suppress': True,       # 阻塞按键（防止传递到其他程序）
            'hold_mode': True,      # True=长按模式, False=单击切换模式
            'enabled': True         # 启用此快捷键
        },
        # F8 单击切换模式（按一次开始录音，再按一次停止）
        {
            'key': 'f8',
            'type': 'keyboard',
            'suppress': True,
            'hold_mode': False,     # 单击切换模式
            'enabled': True
        },
        # 鼠标侧键 X2（Linux 上可能不可用，默认禁用）
        {
            'key': 'x2',
            'type': 'mouse',
            'suppress': True,
...
快捷键配置字段说明：
  ...
  hold_mode  - 录音模式:
               True  = 长按模式（按住录音，松开停止）—— 适合短句输入
               False = 单击切换模式（按一次开始，再按一次停止）—— 适合长段发言
  enabled    - 是否启用此快捷键

长按模式说明：
  - 按住快捷键开始录音，松开停止识别
  - 短按（< threshold 秒）自动补发按键，不影响按键原有功能
  - 推荐用于功能键 (F1-F12)、鼠标侧键等不影响打字的按键

单击切换模式说明：
  - 按一次开始录音，再按一次停止并识别
  - 适合需要长时间连续说话的场合（会议记录、讲座笔记等）
  - 录音过程中再次按下同一快捷键即可停止

阻塞模式说明：
  - 阻塞模式  ：按键事件不会传递给其他程序，避免误触
...
示例配置：
  # 长按 F7 录音，松开停止（默认）
  {'key': 'f7',  'type': 'keyboard', 'suppress': True,  'hold_mode': True,  'enabled': True},

  # 单击 F8 开始录音，再按 F8 停止（切换模式）
  {'key': 'f8',  'type': 'keyboard', 'suppress': True,  'hold_mode': False, 'enabled': True},
